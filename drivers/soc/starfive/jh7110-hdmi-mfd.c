...
static void starfive_hdmi_mfd_clk_disable(void *data)
{
	clk_disable_unprepare(data);
}

static void starfive_hdmi_mfd_rst_assert(void *data)
{
...
static int starfive_hdmi_mfd_probe(struct platform_device *pdev)
{
	 ...
	 * register region, and this MFD's PHY child is the first device in
	...
	ret = devm_add_action_or_reset(dev, starfive_hdmi_mfd_clk_disable, bus_clk);
	if (ret)
		return ret;
	...
	ret = devm_add_action_or_reset(dev, starfive_hdmi_mfd_rst_assert, bus_rst);
	if (ret)
		return ret;
